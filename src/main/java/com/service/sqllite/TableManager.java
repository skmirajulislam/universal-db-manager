...
class TableManager {
    ...
    private boolean tableExists(String tableName) throws SQLException {
        ...
        // Removed .toLowerCase() as SQLite metadata is generally case-insensitive but
    ...
    public void createTable(String tableName) {
        try {
            ...
            // SQLite Dialect Change: SERIAL -> INTEGER PRIMARY KEY AUTOINCREMENT
            String sql = "CREATE TABLE " + tableName + " (id INTEGER PRIMARY KEY AUTOINCREMENT, name VARCHAR(100))";
        ...
        try {
            ...
            try (ResultSet resultSet = metaData.getTables(null, null, "%", new String[] { "TABLE" })) {
                ...
                while (resultSet.next()) {
                    ...
                    // SQLite includes internal tables like 'sqlite_sequence', let's filter them out
                    // for a cleaner look
    ...
    public void truncateTable(String tableName) {
        try {
            ...
            // SQLite Dialect Change: No TRUNCATE command, use DELETE FROM
            String sql = "DELETE FROM " + tableName;

            try (Statement statement = connection.createStatement()) {
                statement.execute(sql);
                System.out.println("Success: Table '" + tableName + "' truncated (cleared) successfully.");
            }
        } catch (SQLException e) {
